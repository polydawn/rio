...
// (e.g. keep-by-default).
func ApplyMaterializeFilters(fmeta *fs.Metadata, filters api.FilesetFilters) {
...
func ApplyScanFilters(fmeta *fs.Metadata, filters api.FilesetFilters) {
	if !filters.FlattenUID.Keep {
		if filters.FlattenUID.Value != nil {
			fmeta.Uid = *filters.FlattenUID.Value
		} else {
			fmeta.Uid = defaults.FilterDefaultUid
		}
	}
	if !filters.FlattenGID.Keep {
		if filters.FlattenGID.Value != nil {
			...
			fmeta.Gid = defaults.FilterDefaultGid
		}
	}
	if !filters.FlattenMtime.Keep {
		if filters.FlattenMtime.Value != nil {
			...
			fmeta.Mtime = defaults.FilterDefaultMtime
		}
	}
}
