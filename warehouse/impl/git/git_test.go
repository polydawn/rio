...
<from git help clone>
       ...
       addition, ftp, and ftps can be used for fetching and rsync
       can be used for fetching and pushing, but these are
       ...
       An alternative scp-like syntax may also be used with the
       ssh protocol:

       ·   [user@]host.xz:path/to/repo.git/

       This syntax is only recognized if there are no slashes
       before the first colon. This helps differentiate a local
       path that contains a colon. For example the local path
       foo:bar could be specified as an absolute path or
...
func TestSanitizeRemote(t *testing.T) {
	emptyError := errcat.Errorf(rio.ErrUsage, "empty git remote")
	// localError := errcat.Errorf(rio.ErrUsage, "failed handling local path")
	dashError := errcat.Errorf(rio.ErrUsage, "warehouse host cannot start with '-'")
	unparsableURI := func(x string) error { return errcat.Errorf(rio.ErrUsage, "failed to parse URI: %s", x) }
	emptyPathURI := func(x string) error { return errcat.Errorf(rio.ErrUsage, "warehouse has empty path: %s", x) }
	...
	protocolFtps := "ftps"
	protocolBogus := "bogus"
	protocolGitSsh := "git+ssh"
	pwd, err := os.Getwd()
	if err != nil {
		t.Fatalf("could not get working directory: %s", err.Error())
	}
	absPwd := riofs.MustAbsolutePath(pwd)
	singleSegmentRel := riofs.MustRelPath("copy")
	multiSegmentRel := riofs.MustRelPath("relative/segments/here")

	/*
		These tests are not necessarily documenting _correct_ behaviour as much as they
		are documenting _actual_ behavior. We'll figure out what it _should_ do as needed.
	*/
	testItems := []struct {
		in       string
		protocol string
		...
		{"%^*!@#$%^&*(){}[];,./<>?", protocolFile, filepath.Join(absPwd.String(), "%^*!@#$%^&*(){}[];,./<>?"), nil},
		{"%^*!@#$%^&*(){}[]:;,./<>?", protocolSsh, "%^*!@#$%^&*(){}[]:;,./<>?", nil}, // it's the colon of URIs
		{"http://%^*", protocolHTTP, "http://%^*", unparsableURI(`permanent client error: parse http://%^*: invalid URL escape "%^*"`)},
		{"https://%^*", protocolHTTP, "https://%^*", unparsableURI(`permanent client error: parse https://%^*: invalid URL escape "%^*"`)},
		{"git://%^*", protocolHTTP, "git://%^*", unparsableURI(`permanent client error: parse git://%^*: invalid URL escape "%^*"`)},
		{"ssh://%^*", protocolHTTP, "ssh://%^*", unparsableURI(`permanent client error: parse ssh://%^*: invalid URL escape "%^*"`)},
		{"file://%^*", protocolHTTP, "file://%^*", unparsableURI(`permanent client error: parse file://%^*: invalid URL escape "%^*"`)},
		{"ssh://^", protocolHTTP, "ssh://^", unparsableURI(`permanent client error: parse ssh://^: invalid character "^" in host name`)},
		{"file://^", protocolHTTP, "file://^", unparsableURI(`permanent client error: parse file://^: invalid character "^" in host name`)},
		// try empty protocols
		{"bogus://", protocolBogus, "bogus://", emptyPathURI("bogus://")},
		{"file://", protocolFile, "file://", emptyError},
		{"ftp://", protocolFtp, "ftp://", emptyPathURI("ftp://")},
		{"ftps://", protocolFtps, "ftps://", emptyPathURI("ftps://")},
		{"git+ssh://", protocolGitSsh, "git+ssh://", emptyPathURI("git+ssh://")},
		{"git://", protocolGit, "git://", emptyPathURI("git://")},
		{"http://", protocolHTTP, "http://", emptyPathURI("http://")},
		{"https://", protocolHTTPS, "https://", emptyPathURI("https://")},
		{"rsync://", protocolRsync, "rsync://", emptyPathURI("rsync://")},
		{"ssh://", protocolSsh, "ssh://", emptyPathURI("ssh://")},

		// try protocols + '/''
		{"bogus:///", protocolBogus, "bogus:///", nil},
		{"file:///", protocolFile, "/", nil},
		{"ftp:///", protocolFtp, "ftp:///", nil},
		{"ftps:///", protocolFtps, "ftps:///", nil},
		{"git+ssh:///", protocolGitSsh, "git+ssh:///", nil},
		{"git:///", protocolGit, "git:///", nil},
		{"http:///", protocolHTTP, "http:///", nil},
		{"https:///", protocolHTTPS, "https:///", nil},
		{"rsync:///", protocolRsync, "rsync:///", nil},
		{"ssh:///", protocolSsh, "ssh:///", nil},

		// try some local fs
		{"", protocolFile, "", emptyError},
		{"/", protocolFile, "/", nil},
		{"  /  ", protocolFile, "/", nil},
		{"/home/proj/.git", protocolFile, "/home/proj/.git", nil},
		{"file:///home/proj/.git", protocolFile, "/home/proj/.git", nil},

		// relative paths with one segment
		{"../copy", protocolFile, absPwd.Dir().Join(singleSegmentRel).String(), nil},
		{"./copy", protocolFile, absPwd.Join(singleSegmentRel).String(), nil},
		{"copy", protocolFile, absPwd.Join(singleSegmentRel).String(), nil},
		{"-copy", protocolFile, filepath.Join(absPwd.String(), "-copy"), nil},
		{"file://../copy", protocolFile, absPwd.Dir().Join(singleSegmentRel).String(), nil},
		{"file://./copy", protocolFile, absPwd.Join(singleSegmentRel).String(), nil},
		{"file://copy", protocolFile, absPwd.Join(singleSegmentRel).String(), nil},
		{"file://-copy", protocolFile, filepath.Join(absPwd.String(), "-copy"), nil},
		{"FILE://../copy", protocolFile, absPwd.Dir().Join(singleSegmentRel).String(), nil},
		{"FILE://./copy", protocolFile, absPwd.Join(singleSegmentRel).String(), nil},
		{"FILE://copy", protocolFile, absPwd.Join(singleSegmentRel).String(), nil},
		{"FILE://-copy", protocolFile, filepath.Join(absPwd.String(), "-copy"), nil},
		...
		{"../relative/segments/here", protocolFile, absPwd.Dir().Join(multiSegmentRel).String(), nil},
		{"./relative/segments/here", protocolFile, absPwd.Join(multiSegmentRel).String(), nil},
		{"relative/segments/here", protocolFile, absPwd.Join(multiSegmentRel).String(), nil},
		{"-relative/segments/here", protocolFile, filepath.Join(absPwd.String(), "-relative/segments/here"), nil},
		{"file://../relative/segments/here", protocolFile, absPwd.Dir().Join(multiSegmentRel).String(), nil},
		{"file://./relative/segments/here", protocolFile, absPwd.Join(multiSegmentRel).String(), nil},
		{"file://relative/segments/here", protocolFile, absPwd.Join(multiSegmentRel).String(), nil},
		{"file://-relative/segments/here", protocolFile, filepath.Join(absPwd.String(), "-relative/segments/here"), nil},
		{"FILE://../relative/segments/here", protocolFile, absPwd.Dir().Join(multiSegmentRel).String(), nil},
		{"FILE://./relative/segments/here", protocolFile, absPwd.Join(multiSegmentRel).String(), nil},
		{"FILE://relative/segments/here", protocolFile, absPwd.Join(multiSegmentRel).String(), nil},
		{"FILE://-relative/segments/here", protocolFile, filepath.Join(absPwd.String(), "-relative/segments/here"), nil},

		// try a bunch of protocols
		{"git://git.kernel.org/pub/scm/.../linux.git my-linux", protocolGit, "git://git.kernel.org/pub/scm/.../linux.git%20my-linux", nil},
		{"git+ssh://git.kernel.org/pub/scm/.../linux.git my-linux", protocolGitSsh, "git+ssh://git.kernel.org/pub/scm/.../linux.git%20my-linux", nil},
		{"ssh://git.kernel.org/pub/scm/.../linux.git my-linux", protocolSsh, "ssh://git.kernel.org/pub/scm/.../linux.git%20my-linux", nil},
		{"rsync://git.kernel.org/pub/scm/.../linux.git my-linux", protocolRsync, "rsync://git.kernel.org/pub/scm/.../linux.git%20my-linux", nil},
		{"ftp://git.kernel.org/pub/scm/.../linux.git my-linux", protocolFtp, "ftp://git.kernel.org/pub/scm/.../linux.git%20my-linux", nil},
		{"ftps://git.kernel.org/pub/scm/.../linux.git my-linux", protocolFtps, "ftps://git.kernel.org/pub/scm/.../linux.git%20my-linux", nil},
		// turns out the git library doesn't really detect silly schemes...
		{"bogus://git.kernel.org/pub/scm/.../linux.git my-linux", protocolBogus, "bogus://git.kernel.org/pub/scm/.../linux.git%20my-linux", nil},
		// github http converts to https
		{"http://github.com/polydawn/rio", protocolHTTPS, "https://github.com/polydawn/rio", nil},
		// try some other http/https urls
		{"https://go.googlesource.com/go", protocolHTTPS, "https://go.googlesource.com/go", nil},
		{"http://go.googlesource.com/go", protocolHTTP, "http://go.googlesource.com/go", nil},
		{"http://-go.googlesource.com/go", "", "", dashError},
		// we don't really care about testing auth here.
		{"http://user:pass@go.googlesource.com/go?foo", protocolHTTP, "http://user:pass@go.googlesource.com/go?foo", nil},
	}
	for idx, item := range testItems {
		t.Run(fmt.Sprintf("remote URI[%d]: %s", idx, item.in), func(t *testing.T) {
			remote, err := SanitizeRemote(item.in)
			errString := fmt.Sprintf("%#v", err)
			expectedErrString := fmt.Sprintf("%#v", item.err)
			if errString != expectedErrString {
				t.Errorf("expected %#v but got %#v", expectedErrString, errString)
			}
			if err == nil {
				...
				if remote != "" {
					t.Errorf("expected remote to be empty when there is an error")
				}
			}
		})
	}
